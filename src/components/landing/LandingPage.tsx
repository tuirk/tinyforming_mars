...
export function LandingPage() {
  ...
  async function handleGuest() {
    ...
    try {
      ...
      await createUserDocument(result.user);
    } catch { setError('Failed to start guest session.'); }
    finally { setLoading(false); }
  }
  ...
  return (
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', background: '#0d0d1a', position: 'relative', overflow: 'hidden' }}>
      ...
      {/* Mars decoration */}
      <div style={{ position: 'absolute', right: '-120px', top: '50%', transform: 'translateY(-50%)', width: '700px', height: '700px', borderRadius: '50%', background: 'radial-gradient(circle at 40% 40%, #C4623A 0%, #8B3D22 50%, #3A1808 100%)', opacity: 0.25, pointerEvents: 'none' }} />
      <div style={{ position: 'absolute', right: '-170px', top: '50%', transform: 'translateY(-50%)', width: '800px', height: '800px', borderRadius: '50%', border: '1px solid #8B3D22', opacity: 0.12, pointerEvents: 'none' }} />
      <div style={{ position: 'absolute', right: '-220px', top: '50%', transform: 'translateY(-50%)', width: '900px', height: '900px', borderRadius: '50%', border: '1px solid #6B2D18', opacity: 0.06, pointerEvents: 'none' }} />
      {/* Stars */}
      {[[15,20,3,0.12],[8,60,2,0.08],[35,15,4,0.1],[25,75,2,0.15],[50,8,3,0.09]].map(([l,t,s,o], i) => (
        <div key={i} style={{ position: 'absolute', left: `${l}%`, top: `${t}%`, width: `${s}px`, height: `${s}px`, borderRadius: '50%', background: '#fff', opacity: o, pointerEvents: 'none' }} />
      ))}
      ...
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px 24px', gap: '60px', position: 'relative', zIndex: 1, flexWrap: 'wrap' }}>
        ...
        <div style={{ maxWidth: '380px', flexShrink: 0 }}>
          ...
          <div style={{ fontSize: '10px', color: '#3a3a5e', fontFamily: "'Inter', system-ui, sans-serif", lineHeight: 1.8 }}>
            <span>Based on the game by <a href="https://boardgamegeek.com/boardgame/282493/tinyforming-mars" target="_blank" rel="noopener noreferrer" style={{ color: '#E8872D', textDecoration: 'none' }}>Michael Bevilacqua</a></span><br />
          ...
          {!showForgot && (
            <>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                ...
                <button onClick={handleGuest} disabled={loading} style={{ ...inputStyle, cursor: 'pointer', textAlign: 'center', background: '#1a1a2e', color: '#E8872D' }}>
                  + Play as guest
                </button>
              </div>
